...
public final class NameProtectManager {
    ...
    public static void setReplacement(String value) {
    ...
    public static boolean isEnabled() {
        return mode != NameProtectMode.OFF && !replacement.isBlank();
    }
    ...
    public static boolean targets(PlayerEntity player) {
        return player != null && targets(player.getGameProfile().name());
    }
    ...
    public static boolean shouldReplaceSelfSkin() {
        MinecraftClient client = MinecraftClient.getInstance();
        return client.player != null && mode == NameProtectMode.SELF && isEnabled();
    }

    public static Text replacementText(PlayerListEntry entry) {
        return Text.literal(replacement);
    }

    public static Text replacementText(String original) {
        if (replacement.isBlank()) return Text.literal(original == null ? "" : original);
        return Text.literal(replacement);
    }
    ...
    public static Text transformChat(Text input) {
        ...
        List<String> names = targetNamesForChat();
        for (String name : names) {
            if (!name.isEmpty() && !name.equals(replacement)) {
                transformed = transformed.replace(name, replacement);
            }
        }
    ...
    private static List<String> targetNamesForChat() {
        ...
        if (mode == NameProtectMode.ALL && client.getNetworkHandler() != null) {
            ...
            for (PlayerListEntry entry : client.getNetworkHandler().getPlayerList()) {
                result.add(entry.getProfile().name());
            }
            return result;
        }
    ...
    public static Text friendNameText(String original) {
        if (!isFriend(original)) return Text.literal(original);
